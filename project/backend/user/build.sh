...
if [ "$docker" = true ]; then
    ...
    docker build --no-cache --build-arg VERSION=0.1.0 -t valentinconan/share-backend-user:master .
fi

if [ "$native" = true ]; then
    echo -e "${yellow}Building native docker image...${default}"
    npm run native
    docker build --no-cache --build-arg VERSION=0.1.0  -f Dockerfile.native -t valentinconan/share-backend-user-native:master .
fi
